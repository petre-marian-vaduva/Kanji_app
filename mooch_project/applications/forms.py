...
class requestipmappupgradeform(ModelForm):
    class Meta:
        model = Runtime
        fields = ['instance_name','Server','app_version']

    app_version = forms.ModelChoiceField(queryset=ipm_appversions.objects.all().order_by('-id'),widget=forms.Select(attrs={'class':'custom-select mb-2 mr-sm-2 mb-sm-0'}),error_messages={'required': 'Application Version is Required'}) 
...
class requestlorappupgradeform(ModelForm):
    ...
    app_version = forms.ModelChoiceField(queryset=lor_appversions.objects.all().order_by('-id'),widget=forms.Select(attrs={'class':'custom-select mb-2 mr-sm-2 mb-sm-0'}),error_messages={'required': 'Application Version is Required'}) 
